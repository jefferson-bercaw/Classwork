...
def send_data_to_server(patient_name, patient_id, blood_string):
...
def check_and_upload_data(patient_name, patient_id, blood_letter,
                                        rh):
    ...
    message = send_data_to_server(patient_name, patient_id, blood_string)
...
def set_up_window():
    ...
    def ok_btn_cmd():
        ...
        patient_id = int(id_value.get())
        blood_letter = blood_letter_value.get()
        rh = rh_factor_value.get()
        donation_center = donation_value.get()

        # message = check_and_upload_data(patient_name, patient_id, blood_letter,
        #                                 rh)

        # status_label.configure(text=message)

        id_entry.configure(state=tk.DISABLED)  # deactivate widget
        return


    def cancel_btn_cmd():
        # root.destroy()
        id_entry.configure(state=tk.NORMAL)  # reactivate widget

    def change_label_color():
        current_color = top_label.cget("foreground")
        if current_color == "":  # if default color
            color = "black"
        else:
            color = current_color.string  # if not, get the string

        if color == "red":
            top_label.configure(foreground="black")
        elif color == "black":
            top_label.configure(foreground="red")

        root.after(1000, change_label_color)

    def shuffle_choices():
        current_choices = list(donation_combobox.cget("values"))  # get the current values of the combobox
        import random
    ...
    def change_image_command():
        ...
        # Use tkinter.messagebox to pop up a window that notifies the user.
    ...
    top_label.grid(column=0, row=0, columnspan=2, sticky="W")  # can span over two columns
    ...
    id_entry.grid(column=1, row=2, padx=10)
    ...
    A_check = ttk.Radiobutton(root, text="A", variable=blood_letter_value, value="A")
    A_check.grid(column=0, row=3, sticky="W")

    B_check = ttk.Radiobutton(root, text="B", variable=blood_letter_value, value="B")
    B_check.grid(column=0, row=4, sticky="W")

    AB_check = ttk.Radiobutton(root, text="AB", variable=blood_letter_value, value="AB")
    AB_check.grid(column=0, row=5, sticky="W")

    O_check = ttk.Radiobutton(root, text="O", variable=blood_letter_value, value="O")
    O_check.grid(column=0, row=6, sticky="W")

    rh_factor_value = tk.StringVar()
    rh_factor_value.set("-")
    check_box_widget = ttk.Checkbutton(root, text="Rh Positive", variable=rh_factor_value, onvalue="+", offvalue="-")
    ...
    donation_combobox["values"] = ("Durham", "Greensboro", "Apex", "Raleigh")
    donation_combobox.state(["readonly"])  # prevent user from entering custom location
    donation_combobox["values"] = ("Augusta", "Atlanta", "Athens")  # can dynamically change locations

    donation_combobox.configure(postcommand=shuffle_choices)  # when you first interact with the widget, run this
    status_label = ttk.Label(root, text="")
    status_label.grid(column=0, row=20)

    pil_image = Image.open("pic.jpg")
    pil_image = pil_image.resize((192, 108))
    tk_image = ImageTk.PhotoImage(pil_image)
    image_label = ttk.Label(root, image=tk_image)
    image_label.grid(column=1, row=7)

    image_change_button = ttk.Button(root, text="Change Image", command=change_image_command)
